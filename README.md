...
たとえば `benefit_valid_days = 3` なら、前回チェックイン日 + 3日目の 23:59:59 まで対象です。
...
この値は集客の主要調整変数です。
短くすると特典対象は絞られます。
長くすると再来店の心理的ハードルは下がりますが、値引き対象は増えます。
...
- 再来店特典の有効日数
- 特典対象期限
- 今日のチェックインが初回、特典対象、通常チェックイン、使用済みのどれか
- 次回の特典対象期限
...
通常粗利400円、値引き80円、値引き後粗利320円の場合、施策による利益増減は次のように考えます。

```text
施策による利益増減 = 400 × 追加販売数 - 80 × 特典使用数
```
...
つまり、特典使用者数の20%を超える追加販売が発生すれば、粗利ベースでは黒字化します。
...
このコードは強固な認証ではなく、店頭で案内する簡易的な合言葉です。
...
- 特典使用済み時は、状態カードに「この80円引きは使用済みです。」を表示し、benefitNoteには同じ文言を出しません
...
- 特典利用率と新規来店数を比較し、80円値引きが広告費として成立しているか分析する
- 状態別アイコン表示を追加する
- 状態別に背景色を変える
- 期限日までの残り日数を表示する
- スタンプカード風UIにする
...
1. `current/app-script/Code.gs` を Apps Script へ反映する
2. `settings` シートに `benefit_valid_days` が無い状態で getCustomerState を呼ぶ
3. デフォルト3として動くことを確認する
4. `setBenefitValidDaysForAdmin(2)` または `setBenefitValidDaysTo2ForAdmin()` で2日に変更する
5. `settings` シートに `benefit_valid_days = 2` が入ることを確認する
6. 前回チェックイン日を2日前にして、特典対象になることを確認する
7. 前回チェックイン日を3日前にして、2日設定では期限切れになることを確認する
8. `setBenefitValidDaysForAdmin(3)` または `setBenefitValidDaysTo3ForAdmin()` で3日に戻す
9. 前回チェックイン日を3日前にして、特典対象になることを確認する
10. `index.html` を変更せずに判定が変わることを確認する
